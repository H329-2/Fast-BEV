...
class FastBEV_RS(BaseDetector):
    ...
    def _compute_projection(img_meta, stride, noise=0):
        ...
        intrinsic[:2] /= stride
        # map(func, iter) 用func调用iter，生成一个与iter同型的结果列表。
        # 此处将img_meta转换为tensor列表
    ...
    def extract_feat(self, img, img_metas, mode):
        ...
        # in v1 the shape is (1)
        mlvl_volumes = []
        for lvl, mlvl_feat in enumerate(mlvl_feats):  
            stride_i = math.ceil(img.shape[-1] / mlvl_feat.shape[-1])  # P4 880 / 32 = 27.5
            # [bs*seq*nv, c, h, w] -> [bs, seq*nv, c, h, w] nv is 1/6
            mlvl_feat = mlvl_feat.reshape([batch_size, -1] + list(mlvl_feat.shape[1:]))
            # [bs, seq*nv, c, h, w] -> list([bs, nv, c, h, w])

            ############
            print(mlvl_feat.shape)
            mlvl_feat_split = torch.split(mlvl_feat, 1, dim=1) # in this part should be deleted
            # print(mlvl_feat_split.shape)
            ############

            # mlvl_f_s.shape = [1*[bs, c, h, w]]
            volume_list = []
        ...
        if self.style in ['v1', 'v2']: # this strench is selected
